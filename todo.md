...
이 문서는 현재 `vtol_vision` 개발 상태와 다음 작업을 이어받기 위한 작업 목록입니다.

## 현재 개발 상태

- ROS2 Humble 기반 C++ 비전 노드가 구성되어 있음.
- 카메라 입력, ArUco 탐지, YOLO TensorRT 추론 루프가 분리되어 있음.
- 주요 발행 토픽:
...
- 신규 개발자 문서, Jetson 배포 문서, 타 부서 연동 문서가 한국어로 정리되어 있음.
...
- Jetson 접속, 프로비저닝, 레이턴시 벤치 보조 스크립트가 `tools/` 아래에 있음.
...
- [ ] YOLO 학습 스크립트를 3-class 데이터셋 기준으로 정리
